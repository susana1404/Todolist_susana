...
if (isset($_GET['undone_task'])) {
    $task_id = intval($_GET['undone_task']);
    mysqli_query($koneksi, "UPDATE tasks SET status='0' WHERE id='$task_id'");
    echo "<script>window.location.href='index.php';</script>";
}

// Tandai Task Selesai
if (isset($_GET['done_task'])) {
    $task_id = intval($_GET['done_task']);
     ...
     <div class="container text-center"> <br>
            <div class="border p-3 bg-light">
                ...
                <form method="POST">
                    ...
                    <input type="date" name="due_date" class="form-control mt-2" id="addDueDate" required>
            ...
            <tbody>
                ...
                <?php while ($task = mysqli_fetch_assoc($tasks)) : ?>
                    <tr>
                        ...
                        <td><?= $task['due_date']; ?></td>
                        <td>
                            <a href="#" data-bs-toggle="modal" data-bs-target="#editTaskModal<?= $task['id']; ?>" 
                            class="btn btn-warning btn-sm">Edit</a>
                            <a href="?delete_task=<?= $task['id']; ?>" class="btn btn-danger btn-sm"
                             onclick="return confirm('Yakin ingin menghapus task ini?')">Delete</a>

                             <?php if ($task['status'] == 0) : ?>
    <a href="?done_task=<?= $task['id']; ?>" class="btn btn-primary btn-sm">Tandai Selesai</a>
<?php else: ?>
    <a href="?undone_task=<?= $task['id']; ?>" class="btn btn-secondary btn-sm">Batalkan Selesai</a>
<?php endif; ?>
...
<div class="modal fade" id="editTaskModal<?= $task['id']; ?>" tabindex="-1">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form method="POST">
                    ...
                    <!-- Pilih Tanggal -->
    <input type="date" name="due_date" class="form-control mt-2 due-date-input" 
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Atur batas minimum tanggal di form tambah task
        const addDueDate = document.getElementById('addDueDate');
        if (addDueDate) {
            addDueDate.setAttribute('min', today);
        }

        // Atur batas minimum tanggal di semua form edit task
